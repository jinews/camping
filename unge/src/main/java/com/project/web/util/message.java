...
public class message {
	public String makeSignature(String url, String timestamp, String method, String accessKey, String secretKey) throws NoSuchAlgorithmException, InvalidKeyException {
		...
		try {
			...
			signingKey = new SecretKeySpec(secretKey.getBytes("UTF-8"), "");
	...
	public void sendSMS(String tosend,String comment) {
		...
		String serviceId = "";       // 프로젝트에 할당된 SMS 서비스 ID							// service ID : https://console.ncloud.com/sens/project > Simple & ... > Project > 서비스 ID
	    ...
	    toJson.put("to", tosend);						// Mandatory(필수), messages.to	수신번호, -를 제외한 숫자만 입력 가능
	    toArr.put(toJson);
	    ...
	    bodyJson.put("content",comment);	// Mandatory(필수), 기본 메시지 내용, SMS: 최대 80byte, LMS, MMS: 최대 2000byte
